...
RSpec.describe SourceParser::Ical, :timecop_freeze => Time.parse('2013-03-22T14:05:27Z') do
  ...
  context "when parsing events and their locations" do
    before(:each) do
  ...
  context "when parsing multiple items in an Eventful feed" do
    before(:each) do
      expect(SourceParser::Base).to receive(:read_url).and_return(read_sample('ical_eventful_many.ics'))
      @events = SourceParser.to_abstract_events(:url => "intercepted", :skip_old => false)
    end
  ...
  context "with iCalendar events" do
  ...
  context "when importing events with non-local times" do
  ...
  context "when skipping old events" do
    before(:each) do
      allow(SourceParser::Base).to receive(:read_url).and_return(<<-HERE.strip_heredoc)
        BEGIN:VCALENDAR
