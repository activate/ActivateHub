...
RSpec.describe SourceParser::Hcal, "with hCalendar events" do
  ...
  it "should parse a page with multiple events" do
    ...
    first, second = *events
    expect(first.start_time).to eq Time.zone.parse('2008-1-19')
    expect(first.end_time).to eq Time.zone.parse('2008-01-20')
    expect(second.start_time).to eq Time.zone.parse('2008-2-2')
    expect(second.end_time).to eq Time.zone.parse('2008-02-03')
  end
end
...
RSpec.describe SourceParser::Hcal, "with hCalendar to AbstractLocation parsing" do
  it "should extract an AbstractLocation from an hCalendar text" do
    ...
    expect(abstract_location).to be_a_kind_of(AbstractLocation)
